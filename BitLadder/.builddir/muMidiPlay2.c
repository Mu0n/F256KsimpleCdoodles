...
#include "../src/timer0.h" //contains helper functions I often use
#include "../src/mudispatch.h" //dispatch to various chips
#include "../src/musid.h" //dispatch to various chips
...
//deal with instrument changes no matter what is selected
	if((msg0 & 0xF0) == 0xC0)
		...
		//if it's a MIDI target deal with it
	if((bareChip == 0 || bareChip == 4) && byteCount==2)
	{
	
		POKE(wantAlt?MIDI_FIFO_ALT:MIDI_FIFO, msg0);
		POKE(wantAlt?MIDI_FIFO_ALT:MIDI_FIFO, msg1);
		/*
		if(byteCount == 3) POKE(wantAlt?MIDI_FIFO_ALT:MIDI_FIFO, msg2);
		*/
		return;
		}

	//if not...MIDI with 3 byte count or any other chip
	if(byteCount == 3) {
		uint8_t nonMIDIchan = (chipXChannel[chan]&0xF0)>>4;
		uint8_t hinib = (msg0)&0xF0;
	
		textGotoXY(0,5);
		printf("nonMIDICHAN=%02x hinib=%02x chan=%02x",nonMIDIchan,hinib, chan);
		dispatchNote(hinib == 0x90?true:false, chan, msg1, msg2, false, bareChip,false, nonMIDIchan);
...
void resetTrack(uint32_t BASE_ADDR){
	uint32_t pos;
