...
bool mEn = false;
struct timer_t sprAnim, sprBirdAnim;
...
void dealKey(uint8_t raw)
{
	switch(raw)
	{
		...
		case 0x33: //3 bitmap layer 2
			if(b2) bitmapSetVisible(2,false);
			else bitmapSetVisible(2,true);
			b2=!b2;
			break;
		case 0x73: //S prites
...
textGotoXY(22,ty);
textPrint("      Mu0n's VICKY graphics Demo      ");
ty+=3;
textGotoXY(22,ty++);
textPrint(" M: Mouse                             ");
textGotoXY(22,ty++);
textPrint(" B: Border                            ");
textGotoXY(22,ty++);
textPrint(" T: Text or X: memteXt Layer          ");
textGotoXY(22,ty++);
textPrint(" 1: Bitmap Layer 0 or Q: Tile Layer 0 ");
textGotoXY(22,ty++);
textPrint(" 2: Bitmap Layer 1 or W: Tile Layer 1 ");
textGotoXY(22,ty++);
