...
void chainEvent(MIDP *theOne, uint8_t track)
{
...
//play stuff
		...
		while(true) 
			...
			for(uint16_t i = 0; i < theOne.nbTracks; i++)
				...
				if(theOne.tracks[i].delta == 0x00000000) //do the first 0-delay one immediately
					{
					lowest = 0;
					lowestIndex = i;
					...
					chainEvent(&theOne, i);
					i--; //come back to the same track! give it a chance to chain other 0 -delay ones 
				...
				else if(theOne.tracks[i].delta < lowest) //otherwise find the event planned for execution the soonest
					{
			...
			//			
			//when we reach this point, a delay has to be created
			//				
