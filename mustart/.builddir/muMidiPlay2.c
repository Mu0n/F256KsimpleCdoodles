...
uint8_t fudge; //prevents specific .mid file early glitch notes
uint8_t doneWithFudge;
...
//deal with instrument changes no matter what is selected
	...
	if((bareChip == 0 || bareChip == 4))
	{
		if(hinib == 0x90 && doneWithFudge == 0)
		{
		if(fudge++ < 1) return;
		else doneWithFudge = 1;
		}
	...
	if(byteCount == 3) {
		...
		dispatchNote(onOrOff, chan, msg1, msg2, false, chipXChannel[chan],false);
