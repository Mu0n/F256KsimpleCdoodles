...
void dispatchNote(bool isOn, uint8_t channel, uint8_t note, uint8_t speed, bool wantAlt, uint8_t whichChip, bool isBeat){
	...
	if(bareChip==2) //PSG
	{
		...
		if(isOn && note >44) {
				...
				//exceptions
					psgNoteOn( polyPSGChanBits[subChan], //used to correctly address the channel in the PSG command
		...
		{
			psgNoteOff(polyPSGChanBits[subChan], psgTarget);
		}
		return;
	}
	if(bareChip==3) //OPL3
	{
		...
		uint8_t subChan=(whichChip)>>4; //get the destination voice/channel of the target chip, use hi nibble only
		if(isOn) {
				opl3_note(subChan, opl3_fnums[(note+5)%12], (note+5)/12-2, true);
			}
		else
		{
			opl3_note(subChan, opl3_fnums[(note+5)%12], (note+5)/12-2, false);
		}
		return;
	}
