...
{
	for(uint8_t i = 0; i<18; i++) selectableChan[i]=1;
}
void clampOP4(void)
{
	if(chip_OPL3_PAIRS == 0x00) return;
	
	for(uint8_t i = 0; i<18; i++) selectableChan[i]=0; //disable all by default
	selectableChan[0] = 1;
	selectableChan[1] = 1;
	selectableChan[2] = 1;
	selectableChan[9] = 1;
...
typedef struct opl3Instrument {
   ...
   */
	...
	gPtr->OPL3Values->OP2_WAV     = opl3_instrument_defs[1].OP2_WAV;
	gPtr->OPL3Values->OP1_WAV     = opl3_instrument_defs[1].OP1_WAV;
...
int8_t findFreeChannel(uint8_t *ptr, uint8_t howManyChans)
	...
	for(i=0;i<howManyChans;i++)
		{
		if(ptr[i] == 0 && selectableChan[i]) return i;
		}
	return -1;
	}
